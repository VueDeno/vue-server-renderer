function mainFunc(global, factory) {
  /*  */
  ...
  function getStyle(vnode, checkChild) {
    ...
    while ((parentNode = parentNode.parent)) {
      if (
        parentNode.data && (styleData = normalizeStyleData(parentNode.data))
      ) {
        extend(res, styleData);
      }
    }
  ...
  function genScopedSlots(
    ...
    return ("scopedSlots:_u([" + generatedSlots + "]" + (needsForceUpdate
      ? ",null,true"
      : "") +
  ...
  function genComponent(
    ...
    var children = el.inlineTemplate ? null : genChildren(el, state, true);
    return ("_c(" + componentName + "," + (genData$2(el, state)) + (children
      ? ("," + children)
      : "") +
      ")");
  }
  ...
  function genStyleSegments(
    ...
    if (staticStyle && !styleBinding && !vShowExpression) {
      ...
      return [{
        type: EXPRESSION,
        value:
          ("_ssrStyle(" + (parsedStaticStyle || "null") + "," +
  ...
  function checkIdentifier(
    ...
    if (typeof ident === "string") {
      try {
        ...
        warn(
          ("invalid " + type + ' "' + ident + '" in expression: ' +
            (text.trim())),
          range,
        );
      }
  ...
  function checkExpression(exp, text, warn, range) {
    try {
      ...
      var keywordMatch = exp.replace(stripStringRE, "").match(
        prohibitedKeywordRE,
      );
      if (keywordMatch) {
        warn(
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (count >= start) {
        for (var j = i - range; j <= i + range || end > count; j++) {
          ...
          res.push(
            ("" + (j + 1) + (repeat$1(" ", 3 - String(j + 1).length)) + "|  " +
              (lines[j])),
          );
  ...
  function createCompileToFunctionFn(compile) {
    ...
    return function compileToFunctions(
      ...
      {
        if (
          (!compiled.errors || !compiled.errors.length) && fnGenErrors.length
        ) {
          warn$$1(
  ...
  function _traverse(val, seen) {
    ...
    if (
      (!isA && !isObject(val)) || Object.isFrozen(val) || val instanceof VNode
    ) {
      return;
    }
    if (val.__ob__) {
  ...
  function resolveAsyncComponent(
    ...
    if (
      owner && isDef(factory.owners) && factory.owners.indexOf(owner) === -1
    ) {
      // already pending
